...
static_lines2 = [pm.Segment(static_body, (0.0, 60.0), (0.0, 800.0), 0.0)]
...
def remove_everything():
    ...
    for block in block_to_remove:
        space.remove(block.static_line_top)
        space.remove(block.static_line_bottom)
        space.remove(block.static_line_right)
        space.remove(block.static_line_left)
...
load_music()
level = Level(pigs, columns, beams,static_block, space)
level.number = 0
level.load_level()
...
while running:
    ...
    screen.blit(background[level.number % no_of_levels], (0, 0))
    #gravity condition
    if level.number == 7 or level.number == 8:
        space.gravity = (0.0, -10)
    else:
        space.gravity = (0.0, -800)
